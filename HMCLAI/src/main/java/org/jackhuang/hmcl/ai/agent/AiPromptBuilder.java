...
public final class AiPromptBuilder {
    ...
    private static final String TOOLS_GUIDE = String.join("\n",
            ...
            "- Mod maintenance: delete_mod(mod[, instance]) remove one mod file (recycle-bin when possible; refuses if the name matches 0 or >1 files). update_mod(mod[, instance, source]) update one installed mod to its newest compatible build and delete the old jar (avoids duplicate-mod launch crashes; run check_mod_updates first).",
            ...
            "- World backups (versioned full-copy snapshots + retention N; NOT incremental/git): create_world_backup(world[, instance]) make a timestamped snapshot & prune old ones; list_world_backups(world[, instance]) list snapshots (newest first, with ids); restore_world_backup(world, backupId[, instance]) roll a world back to a snapshot (DESTRUCTIVE/red-confirm; auto-backs-up current world first). Prefer create_world_backup over the older backup_world.",
            ...
            "Accounts / Java:",
            "- list_accounts, add_offline_account(username), select_account(username), microsoft_login (native OAuth dialog). set_skin([username,] source/skinPath/cslApi[, capePath, model]): set an account's skin (+cape for offline accounts) — offline supports local PNG / LittleSkin / CSL skin-station / steve/alex presets; online (Microsoft/authlib-injector) accounts support a local PNG upload only. list_java: discovered Java runtimes.",
